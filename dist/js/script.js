...
function calc() {
  ...
  let sex, ratio, height, weight, age; //! Получаем из LS ранее сохраненное значение sex и ratio.
  ...
  function getDinamicInformation(selector) {
    ...
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        //! Если ползаватель ввел не число.
...
function cards() {
  ...
  Object(_sevices_services__WEBPACK_IMPORTED_MODULE_0__["getResource"])('http://localhost:3000/menu').then(data => {
    data.forEach(({
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); //! собираем данные из формы в обьект FormData.

      const formData = new FormData(form); //! Перегоняем FormData в Object.
...
function slider({
  ...
  next.addEventListener('click', () => {
    ...
    slidesField.style.transform = `translateX(-${offset}px)`; //! Изменяем текущий слайд, если индекс равет общему колл-ву, 
    //! возвращаем в начало. 
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParent = document.querySelector(tabsParentSelector); //? Фн изначально скрывает контент и активные табы на странтице.
  ...
  hideTabsContent();
  showTabContent(); //! Делегирование события: клик на родительском элементе.
